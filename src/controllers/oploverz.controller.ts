...
const oploverzController = {
  ...
  async getAnimeDirectory(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (hasFilters) {
        ...
        if (query.genre) query.genre.split(",").forEach((g) => params.append("genre[]", g));
        if (query.season) query.season.split(",").forEach((s) => params.append("season[]", s));
        if (query.studio) query.studio.split(",").forEach((s) => params.append("studio[]", s));
